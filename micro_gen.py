...
def lhs_to_micro(lhs_val):
    ...
    X = make_microstructure(n_samples=1, size=(
        ds, ds, ds), n_phases=2, grain_size=(gx, gy, gz), volume_fraction=vf, seed=seed)
...
def save_micros(micros, metadata=None, fbase="paper_micro_full", samples_per_file=200):
    ...
    for i in range(0, num_samples, samples_per_file):
        ...
        dset = output_f.create_dataset('micros', data=mic_i, compression='gzip', compression_opts=6, dtype=int)
...
def main():
    ...
    if plot_samp:
        ...
        plt.figure()
        plt.hist(gx_vals, bins=np.linspace(0.5, ds + 0.5, ds + 1), density=True, ec='black')
        plt.xlabel("gx")
