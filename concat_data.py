...
def concat_files(basename, output_file, keys):
        ...
        for key in keys:
            ...
            if curr_dataset is not None:
                if big_file.get(key) is None:
                    ...
                    big_file.create_dataset(key, shape=curr_dataset.shape, dtype=curr_dataset.dtype, compression='gzip', compression_opts=6, maxshape=(None,) + curr_dataset.shape[1:], chunks = chunk_size)
    ...
    for key in keys:
        ...
        del big_file[key]
        big_file.create_dataset(key, data=data, compression='gzip', compression_opts=6, chunks = chunks)
...
concat_files(f'{output_dir}/{basename}', f'{basename}_responses.h5', ['stress', 'strain'])
