...
parser.add_argument('-bc_component', default=0, type=int, choices = [0,1,2,3,4,5], help='Which direction should we strain for the appplied BCs (in Abaqus ordering)?')
...
def generate_abaqus_mesh(ds, applied_strain, bc_component, save_dir=None):
    ...
    for k in range(ds):
        for j in range(ds):
            for i in range(ds):
                ...
                idx += 1
                elements.append([idx, skip + node + dn, skip + node, node, 
                    node + dn, skip + node + dn + 1, skip + node + 1, node+1, 
                    node + dn + 1
                    ])
    ...
    node_sets = [
        sxp, sxm, szp, szm, syp, sym, xpyp, xpym, xmyp, xmym, zpyp, 
